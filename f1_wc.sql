...
-- First thing first, let's double check for any duplicates in the tables. I proceeded to run this same query with all tables to check for duplicates.

with circuits_dup_check AS(
SELECT
	circuitId
    ,circuitRef
    ,name
...
SELECT 
	CONCAT(d.forename," ",d.surname) as NAME
    ,COALESCE(MAX(d.number),'N/A')AS NUM
    ,MAX(d.dob) AS DOB
    ,MAX(d.nationality) AS NATIONALITY
...
FROM results AS re
	...
	LEFT JOIN circuits AS c
		ON c.circuitId = r.circuitId
	GROUP BY 1
    ORDER BY COALESCE(MIN(re.fastestLapTime), 'No Time Registered') ASC;
...
SELECT 
	r.name AS RACE
	,st.status AS STATUS
	,COUNT(re.statusId) STATUS_COUNT
FROM results AS re
	LEFT JOIN races AS r
		ON r.raceId = re.raceId
	LEFT JOIN status AS st
		ON re.statusId = st.statusId
	WHERE re.statusId NOT IN ('1','11','12','13','14','15','16','17','18','19')
	GROUP BY 1,2
    ORDER BY r.name ASC, COUNT(re.statusId) DESC;
